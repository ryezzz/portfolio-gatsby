...
const particles =(n,width,height)=> Array.from({ length: n }, () => [
  Math.random() * width,
  Math.random() * height,
]);
...
export default ({ children }) => {
  ...
  return (
    <containerDiv>
    <div className="background-div"><VeroniBackground particles={getParticlesLarge} stroke={5} width={responsiveWidth} height={2000} n={40} rgba={rgba(.1)} />
...
<div className="site-content">
      <header className="site-header" >
        <div className="site-title" style={{color:rgba}}>
          <Link style={{color:rgba (1)}} to="/">{data.site.siteMetadata.title}</Link>
        </div>
        <Navigation />
      </header>
